...
public class JestDemoApplication {
    ...
    private static Logger logger = Logger.getLogger(JestDemoApplication.class);
    ...
    public static void createUpdateIndex(JestClient jestClient, String index, String baseType, String mappingJson) throws IOException {
        ...
        if (indexExists) {
            ...
            JestResult res = jestClient.execute(new PutMapping.Builder(index, baseType, mappingJson).build());
            if (!res.isSucceeded()) {
            ...
            JestResult res = jestClient.execute(new CreateIndex.Builder(index).build());
            if (!res.isSucceeded()) {
                logger.error(res.getErrorMessage());
                throw new IOException(res.getErrorMessage());
            }
            res = jestClient.execute(new PutMapping.Builder(index,baseType, mappingJson).build());
            if (!res.isSucceeded()) {
                logger.error(res.getErrorMessage());
                throw new IOException(res.getErrorMessage());
            }
